...
class NetG(nn.Module):
    def __init__(self, ngf, nz, cond_dim, imsize, ch_size):
        ...
        self.to_rgb = nn.Sequential(
            ...
            Involution2d(out_ch, ch_size, 3, 1, 1, sigma=None),
            nn.Tanh(),
            )
...
class NetD(nn.Module):
    def __init__(self, ndf, imsize=128, ch_size=3):
        ...
        self.conv_img = Involution2d(ch_size, ndf, 3, 1, 1, sigma=None)
        # build DBlocks
...
class NetC(nn.Module):
    def __init__(self, ndf, cond_dim=256):
        ...
        self.joint_conv = nn.Sequential(
            Involution2d(ndf*8+cond_dim, ndf*2, 3, 1, 1, bias=False, sigma=None),
            nn.LeakyReLU(0.2,inplace=True),
            Involution2d(ndf*2, 1, 4, 1, 0, bias=False, sigma=None),
        )
...
class G_Block(nn.Module):
    def __init__(self, cond_dim, in_ch, out_ch, upsample):
        ...
        self.c1 = Involution2d(in_ch, out_ch, 3, 1, 1, sigma=None)
        self.c2 = Involution2d(out_ch, out_ch, 3, 1, 1, sigma=None)
        self.fuse1 = DFBLK(cond_dim, in_ch)
        self.fuse2 = DFBLK(cond_dim, out_ch)
        if self.learnable_sc:
            self.c_sc = Involution2d(in_ch,out_ch, 1, stride=1, padding=0, sigma=None)
...
class D_Block(nn.Module):
    def __init__(self, fin, fout, downsample=True):
        ...
        self.conv_r = nn.Sequential(
            Involution2d(fin, fout, 4, 2, 1, bias=False, sigma=None),
            nn.LeakyReLU(0.2, inplace=True),
            Involution2d(fout, fout, 3, 1, 1, bias=False, sigma=None),
            nn.LeakyReLU(0.2, inplace=True),
        )
        self.conv_s = Involution2d(fin,fout, 1, stride=1, padding=0, sigma=None)
