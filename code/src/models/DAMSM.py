...
class XLM_ENCODER(nn.Module):
  def __init__(self):
    super(XLM_ENCODER, self).__init__()
    self.model = XLMModel.from_pretrained('xlm-mlm-en-2048',output_hidden_states=True)

  def forward(self, captions) :
    hidden_states = self.model(input_ids=captions)[1]
...
class RNN_ENCODER(nn.Module):
    ...
    def forward(self, captions, cap_lens, hidden, mask=None):
        ...
        print(output.transpose(1, 2).size(),"word embedding size")
        return words_emb, sent_emb
